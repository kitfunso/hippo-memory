...
- **A quarantined row can no longer be shared to the global store**, even with `--force`, by
  `hippo promote` or by sleep auto-share, and it takes no part in conflict detection, so it cannot mark a visible memory as conflicted.
...
- **Add DolphinBench to the roadmap as TE11, a benchmark to run and verify against.** DolphinBench (Mem0, September 2026) grades the action an agent takes after five years of history and reports cost and latency beside accuracy. The entry fixes the arms (hippo on the paper's Claude Code and Sonnet setup, against Built-In, Mem0 and Honcho), requires our Built-In run to reproduce the paper's row before any hippo number is read, and names two changes the harness needs: recall that writes nothing during tests, and a clock that follows each message's date.
...
- **Record the mechanism audit result, revised after an independent critique: the physics scorer and the 7-day decay default hurt recall; on E1, outcome feedback, retrieval strengthening, a 365-day half-life and the full lifecycle help; sleep's merge and dedup fall below the 3 pp floor.** `docs/evals/2026-09-23-mechanism-audit-result.md` holds every verdict with its paired interval, the critique verbatim, the raw outputs and the scripts that recompute the gates and the critique's dating split. No default changes in this release; each proposal waits for sign-off.
...
- **Round 2 of the mechanism audit is in.** `docs/evals/2026-09-23-mechanism-audit-round2-result.md`
  replicates round 1 exactly from the released code, confirms physics off under the exact
  scorer, finds the search recency factor and sleep's merge cost recall, and shows the outcome
  nudge alone removes marked-bad memories on E1. It corrects a sign in amendment 1's R2b line.
  No product code changes.
- **Amend round 2 of the mechanism audit after a seed collision.** A second decay-default
  registration (`docs/evals/2026-09-24-decay-default-prereg-2.md`) ran on the E1 seeds
  round 2's prereg reserved for R2a-R2d. `docs/evals/2026-09-24-mechanism-audit-round2-amendment-1.md`
  transfers R2b and R2d from that run, moves R2a and R2c to fresh seeds (101-120), and gates
  the 365-day half-life and physics-off defaults at npm release until round 2's independent
  critique signs off. No product code changes.
...
- **Sleep no longer deletes pinned memories.** The junk audit removed a pinned row whose text was too short, and dedup removed a pinned copy when an unpinned twin scored higher. Both now keep it, even when the pin lands while they run, and count only what they actually deleted. A pinned or raw row the audit would have removed is reported as a warning, marked `(pinned, kept)` or `(raw, kept)`.
- **Sleep's final write no longer undoes changes made while it ran.** Sleep loads every row, runs its LLM phases, then writes its results in one batch from the copies it loaded at the start. Those stale copies overwrote whatever changed in between. A pin, `forget` or `supersede` made meanwhile was lost: the pin was dropped, the forgotten row came back, and the superseded row became current again. Sleep's own DAG pass lost its work too: facts lost the link to the summary just built for them, so the next sleep built another summary for the same facts, and a summary rebuilt during the run got its old text back. The batch now writes only the fields sleep changed and takes the rest from the live row, skips a row deleted in the meantime, and never deletes a row pinned in the meantime; the report counts that row as kept, not removed. A change to a fact's strength alone no longer marks its summary for a rebuild.
...
- **A pre-registered test of which lifecycle mechanisms help recall.** `docs/evals/2026-09-23-mechanism-audit-prereg.md` fixes the lanes, the pass and fail rule, and what each verdict changes before any run: physics scoring, the 7-day decay default, outcome feedback, recall strengthening and sleep, each against its own off switch. It runs on local CPU only, through `scripts/e1-lifecycle/compare.mjs`, `benchmarks/longmemeval/paired_hits.mjs` and `benchmarks/longmemeval/build_mech_stores.sh`.
...
- **`index.json` stops being refreshed on every write in 1.46.0.** SQLite (`hippo.db`) holds the store and `index.json` is a mirror of it. From 1.46.0 the mirror comes only from an explicit export. Read the store through the CLI, the MCP server or the HTTP API instead of `index.json`. Correction: 1.46.0 and 1.47.0 still refreshed it on every write; the change landed in the release after 1.47.0, where only `rebuildIndex()` writes it.
...
- **Decision record: hippo never starts, stops or supervises an agent runtime.** Track W's W3 becomes a pull-mode adapter kit and W4 a pull-mode limit migration, so runtimes claim their own cards with `hippo card claim`. The record is `docs/decisions/2026-09-20-no-agent-spawn.md`.
...
- **The publish gate trusts vitest's JSON report, not its exit code.** Under load vitest can exit non-zero on a green suite with `Timeout calling "onTaskUpdate"`, and the old gate could not tell that apart from a red one. `scripts/check-tests-pass.mjs` now passes a non-zero exit only when the report shows zero failed tests and suites, at least one passed test, and every unhandled error is that worker IPC timeout. A missing or unreadable report, an all-skipped run and a failing globalSetup teardown all refuse the publish. `docs/release-policy.md` has the full rule.
...
- **A blank `session_id` in the `hippo context` stdin payload no longer blocks the fallback.** A payload carrying `"session_id": ""` was kept as the caller's id, so the host fallback never ran and the trace stamped NULL. A blank or whitespace-only payload id now counts as absent. A non-blank payload id is used exactly as sent.
- **Two limits to know.** `CLAUDE_CODE_SESSION_ID` is inherited by child processes, so a process that outlives its Claude Code session keeps that id: its `hippo context` calls still owner-match the old session's task snapshot, and its traces still carry the old id. Also, `recall_traces.session_id` now holds two kinds of id in one column: ids you pass (`--session-id`, `HIPPO_SESSION_ID`) and Claude Code session ids. Nothing in hippo groups traces by that column today. An analysis that does should expect one working session to show up under more than one id.
- Pinned by new cases in `tests/recall-trace-wiring.test.ts` and `tests/snapshot-df1-ambient-surfaces.test.ts`. The test run now clears `CLAUDE_CODE_SESSION_ID` in its global setup, so a suite started from a Claude Code shell cannot leak that id into the CLI children it spawns.

## 1.42.0 - 2026-09-19

### Fixed
- **`--reranker cross-encoder` scored every candidate 1.0 and reordered nothing.** The reranker ran `Xenova/ms-marco-MiniLM-L-6-v2` through the Transformers.js `text-classification` pipeline. That model has a single-logit regression head, and the pipeline applies softmax to the length-1 logit vector, which is 1.0 for every input. Every candidate tied, the stable sort kept the input order, and the pass had been a silent no-op since it first shipped. `src/rerankers/cross-encoder.ts` now loads `AutoTokenizer` and `AutoModelForSequenceClassification` directly, tokenizes the query and candidate as a real text pair, reads the raw logit and applies a sigmoid. The old tests could not see the bug: the dynamic import of a bare specifier failed under the vitest module runner, so CI only ever exercised the identity fallback, and the availability probe compared one score against the input score, which a constant-output model passes. The package is now resolved to a file URL before import, the CI `micro-eval` job, which already installs the Transformers.js backend, now also runs `tests/rerankers/cross-encoder.test.ts` with `HIPPO_REQUIRE_CROSS_ENCODER=1` so the real model is scored under vitest on every push and a skipped model fails the step, the probe scores a matching and a nonsense candidate and requires them to differ, the ordering test puts the decoy first so identity ordering fails it, and a live model that scores every candidate the same now fails the suite instead of skipping it. Measured effect of the repair: on a 300-query private developer store R@1 moves from 0.2600 to 0.4133; on LongMemEval (n=500) it does not beat base on any ranking metric; on 150 graded LongMemEval questions it lifts the exact-answer rate by +0.0467 [0.0133, 0.0867] at 5 memories. The optional peer dependency contract is unchanged: with no Transformers.js backend installed the reranker still returns the input order and warns once. Concurrent first calls now share one in-flight model load instead of racing to fetch it twice, a failed load is retried on the next call, and a non-finite score falls back to the prior score instead of silently disabling the sort.

### Added
- **`--reranker jev`: an opt-in reranker backed by the hosted TypeSafe Jev API.** `hippo recall "<query>" --reranker jev` sends the query and the top candidates (each cut to 1,200 characters) to `https://api.typesafe.ai/v1/systemone` in one batched request and reorders them by the returned probabilities. It is off by default, and the default recall path still makes no network call. It needs a `TYPESAFE_API_KEY` environment variable, costs about 0.0004 USD a recall and adds about 300 ms (p50 295 ms, p90 414 ms over 300 calls). `HIPPO_JEV_TIMEOUT_MS` (default 5000) and `HIPPO_JEV_MODEL` (default `jev-latest`) tune it. Any failure (key unset, non-2xx status, timeout, network error, incomplete or out-of-range answers) prints one warning per process and falls back to the local cross-encoder; the reranker never throws and never prints the key. What the evidence supports: Jev ranks better than the repaired cross-encoder on two corpora, R@1 +0.2033 [0.1333, 0.2733] on a 300-query private developer store and +0.0700 [0.0200, 0.1200] on LongMemEval (n=500). What it does not support: a better answer rate. Three graded tests on 150 LongMemEval questions did not show an answer win over the free cross-encoder (+0.0333 [-0.0067, 0.0800] with every arm at 2 memories). Jev at 2 memories ties the cross-encoder at 5, so what it buys today is about 900 fewer context tokens a query. Turning it on sends query and memory text to a third party. Scores are not bit-stable run to run (up to 0.06 on identical input), which makes this the one documented exception to the reranker determinism rule in `src/rerankers/types.ts`. Full numbers, limits and the request shape: `docs/evals/2026-09-19-jev-reranker.md`. The CLI default pool for `--reranker jev` is 40 candidates, the size the evidence was measured at. Pinned by `tests/rerankers/jev.test.ts`, 16 cases over a mocked `fetch`.
...
### Fixed
- **`'stale'` was both a stored confidence tier and a derived one, and every reporting surface collapsed the two.** `resolveConfidence` (`src/memory.ts:447`) returned the literal `'stale'` for any non-pinned, non-verified row untouched for 30 days, discarding the stored tier. `'stale'` is also what `hippo invalidate` and decision supersede store, so a well-grounded `observed` memory nobody had read in a month and a memory a human had explicitly rejected came back identical, and the stored tier was unrecoverable at the call site. `hippo stats` bucketed both under "Stale" and `hippo recall --why` reported `confidence: "stale"` for a row whose stored tier was `observed`. `confidenceFacets` now returns `{ tier, agedOut }` and `confidenceLabel` renders the pair as `observed, aged`, both sitting on one `isAgedOut` predicate so the 30-day threshold and the pinned/verified exemption exist in exactly one place. Eight display sites move onto it: `recall` human and `--why` JSON, `explain --json`, `trace` human and JSON, `status`, the context render, the dashboard payload, and MCP `formatMemories`. `status` gains an `Aged out` line that cross-cuts the four tier buckets rather than partitioning with them, which the label says. The JSON surfaces are a deliberate user-visible change: `confidence` is now the stored tier and `aged_out` carries the rest, which is lossless since the old value is `aged_out ? 'stale' : confidence`. Nothing outside the repo consumes those shapes; the Python SDK types the field as an open string and talks to `src/api.ts`, which never collapsed it. `resolveConfidence` is unchanged, still exported, and keeps two callers: `replay.ts`, which filters and genuinely wants the collapsed answer, and `hippo inspect`, which already printed both facets correctly. In the dashboard, the tier no longer stands in for the age-out, so the confidence checkboxes stopped reaching cold rows; age-out is a separate axis and gets its own `aged out` toggle beside `fading`, and the count is rendered rather than left as a field nothing reads. Removing `'stale'` from the exported `ConfidenceLevel` union is the clean fix and is not taken here: it is a breaking change to a published type plus a migration of the 807 live rows holding the literal, and it stays filed in `TODOS.md`. Pinned by `tests/confidence-display-facets.test.ts`, 16 cases over real stores, plus one aged-out fixture added to the context-render snapshot file with the eight existing snapshots byte-identical; nine mutants were each killed by a different case. One finding came out of building it: `markRetrieved` (`src/search.ts:1254`) refreshes `last_retrieved` for every row a retrieval returns, and `hippo context` renders the post-refresh copy, so that surface can never show an aged row and deliberately carries no `aged_out` field. `hippo recall` renders the pre-refresh entry and does show it. That disagreement predates this change and is filed in `TODOS.md` as its own decision. Codex review then found the new suite carried a date bomb: the fixtures are dated from a fixed `NOW` while `runCli` spawns the real CLI on the real clock, so the row seeded one day old would itself have aged out after 2026-10-06 and the status count would read 2 where the test expects 1. The two clocks are now one. `beforeEach` pins `HIPPO_FAKE_NOW` to that same `NOW`, which `evalNow` (`src/ablation.ts:146`) already honours, and `runCli` spawns off `process.env`, so the in-process dashboard and MCP assertions and the spawned CLI all read the fixture clock. Moving `NOW` a year into the past leaves every case green, which is what the fix buys.
- **Reading a memory undid `hippo invalidate`.** Invalidation weakens a memory by storing `confidence: 'stale'` and tagging it `invalidated` (`src/invalidation.ts:138`), and `markRetrieved` promoted any stored `'stale'` back to `'observed'` on the next read. So one `hippo recall`, one `hippo context`, or one MCP recall put the memory back to a tier the user had explicitly rejected, while the `invalidated` tag stayed on the row. Decision supersede (`src/cli.ts:4703`) is the second writer of a deliberate `'stale'` and was undone the same way. The promotion arrived in `471ba86` as one half of a pair: `consolidate` wrote the derived staleness into the row, and this line woke it on use. The sleep half was removed later, and `src/consolidate.ts:243-245` now states the rule it left behind, that confidence is an epistemic tier and not a cached computation, so `resolveConfidence` derives staleness on read. The read half survived, and with nothing storing a derived `'stale'` any more, undoing a deliberate mark became its only reachable effect. It is deleted rather than made tag-aware, because a guard that reads the `invalidated` tag would re-encode the same overloaded meaning at one more site and need editing for every future writer of a stored `'stale'`. Retrieval still applies its three strengthening effects: `retrieval_count`, `last_retrieved` and the half-life increment. Pinned by `tests/invalidation-survives-recall.test.ts`, which invalidates a real stored memory and reads it back through `getContext`; restoring the promotion fails 4 cases across that file and `tests/search.test.ts`. Measured on two live stores before the fix: 10 rows tagged `invalidated` carried `observed` or `inferred`, with 794 retrievals across the seven in the global store. Existing rows are not repaired by this change; re-run `hippo invalidate --id <id>` on any that matter.
- **A recall served over HTTP never moved `total_recalled`.** `GET /v1/memories` (`src/server.ts:930`) called `api.recall` and nothing incremented, so `hippo stats` reported a number that depended on which surface the caller used. Reproduced on a real server: three memories stored, the route returned 3 results, the counter stayed at 0. The count now lives in the route rather than in `api.recall`, because `api.recall` is not a shared chokepoint: `cmdRecall` runs its own pipeline and never calls it, and MCP calls it for a secondary audit band whose length differs from what the caller is shown, so a counter there would count the wrong thing for MCP. Every other read surface already counts at its own site, including `GET /v1/context`, which routes to `api.getContext`. `hippo recall` has no `runViaServerIfAvailable` branch, so the CLI never routes through this endpoint and the new site cannot double-count it. Drill is still uncounted on both surfaces, which keeps that pair symmetric. Pinned by `tests/http-recall-counter.test.ts`, a real server over a real store; removing the increment fails 4 of its 5 cases. This is the third counter of this class fixed in three releases, after archive in 1.38.7 and plain forget in 1.38.9. The structural change that would end the class, deriving the counters from the audit log instead of incrementing them by hand, is filed in `TODOS.md` rather than taken here: `bootstrapLegacyStore` seeds `total_recalled` with history predating the audit table, so deriving it would silently reset a published number.
- **The release version gate printed OK for a `package-lock.json` it never read.** `check-manifest-versions.mjs` excluded the lockfile wholesale, on the grounds that "the lockfile has its own multi-version semantics". That is true of the `packages["node_modules/..."]` entries and false of the two root fields: `.version` and `.packages[""].version` are the root package's own version, and npm rewrites both on every install. Excluding the whole file let a stale pair through on v1.38.9, where the lockfile stayed at 1.38.8, the gate reported "All 4 lockstep manifests + src/version.ts at version 1.38.9. OK.", and the lockfile was bumped by hand afterwards. The gate now reads the two root fields and still ignores the dependency entries, and a missing lockfile, an unparseable one or a missing version field is reported rather than skipped. The failure message names `npm install --package-lock-only`, since this repo has no `npm version` script to rewrite the lockfile when the manifests are bumped by hand, which is how it drifted. Pinned by `tests/manifest-version-gate.test.ts`, which spawns the real script against a minimal repo in a temp dir and fails 3 of its 5 cases if the new block is reverted. No other manifest was added to the lockstep set: `ui/`, `website/`, `extensions/claude-code-plugin/`, `evals/model-profile-bench.json` and `python/pyproject.toml` all version on their own cadence.
- **A routed `hippo remember` stored the memory twice when the server's socket died after it had already committed the row.** `isConnectionRefused` (`src/client.ts:266`) answered "did the transport fail?", but its only consumer, `runViaServerIfAvailable` (`src/cli.ts:346`), read the answer as "is it safe to replay this write locally?". `ECONNREFUSED` means the request never left the client, so a local replay is safe; a socket that breaks with the request already on the wire means the server may have committed it. The predicate returned true for both, and for a bare `fetch failed` as well, so the CLI cleared the pidfile and wrote a second row on the direct path. Reproduced with a stub server that commits the row then destroys the socket: 2 rows where there should be 1. Node's fetch reports that case as `fetch failed` with `cause.code` `UND_ERR_SOCKET`, a shape the old predicate never named; it fell through the bare-`fetch failed` catch-all to the same wrong answer. `classifyTransportFailure` replaces it with three outcomes: `none`, `never-sent`, `delivery-unknown`. The helper self-heals the pidfile and falls back only on `never-sent`. On `delivery-unknown` it leaves the pidfile alone, tells the user the write may already have landed, and exits non-zero; the next command's connect-phase failure heals the pidfile, so nothing stays stuck. All three callers of the helper (`remember`, `forget`/`archive`, `promote`) are non-idempotent writes, so no per-call-site flag was needed, and `src/cli.ts:349` is the only HTTP-to-direct fallback in the tree. Pinned by `tests/cli-econnreset-no-replay.test.ts`, which fails with 2 rows if the classification is reverted. `isConnectionRefused` was internal (`src/client.ts` is not exported from `src/index.ts`), so it is replaced rather than aliased.
...
### Performance
- **Every prompt loaded both stores in full to return about eleven rows.** The `UserPromptSubmit` hook installs `hippo context --pinned-only --include-recent 5` (`src/hooks.ts:134`), and that branch reads its working set for exactly three things: the empty-store early return, the pins, and the recent-N candidates. It got that set from two `loadAllEntries` calls, so every prompt read all 530 local and 1896 global rows of the real stores off disk and ran the secret-scanning admission predicate over every one of them. Measured on copies of those stores: 67.7ms per prompt, 32.2ms of it inside the two loads. `loadAmbientCandidates` (`src/store.ts`) replaces them on the pinned-only branch, selecting the pins plus a window of the newest rows in one connection. The predicate handed to it is the caller's whole admission rule, DF3's quality floor included, because that floor is applied to the recent-N slice after the load: counting candidates without it let the load call a window of junk rows sufficient and return an empty slice where the old whole-corpus load reached past them. A store whose 32 newest rows all failed the floor returned 0 entries instead of 5 until that was fixed. Interleaved medians over 40 iterations against the same two stores: 72.7ms to 43.4ms with a project set, 90.0ms to 54.7ms without, identical result ids in both. The first attempt was slower than the baseline, 78.3ms, because opening a connection plus `initStore` costs about 5.8ms whatever the row count, so splitting the work into two helper calls paid that floor twice; a widening retry loop that reopened the connection each round reached 118.4ms on a window that mostly failed admission. Both are gone: one connection, and one exhaustive fallback for the case where nothing in the window is admissible. The worst case, a local-only store where every row is rejected so the fallback always runs, measured 37.2ms before and 38.6ms after, which is slower and within noise. No migration was needed: `idx_memories_tenant_created` (`src/db.ts:401`) already serves the window, and the pinned query measured 3.34ms unindexed over 1896 rows. `loadAllEntries` is untouched and still serves every other caller, including the non-pinned `getContext` branch. Pinned by `tests/context-pinned-only-load.test.ts`, 16 cases over real stores; dropping the pinned query, dropping the fallback, flipping the `id DESC` tiebreak, dropping the quality floor from the load predicate, or dropping the pin bypass from it each fails a different case. Codex review then found two compatibility regressions, both reproduced before being fixed. The window ordered `created` as text, which is chronological only for canonical UTC ISO: `deserializeEntry` preserves offset timestamps from legacy markdown, so `2026-06-01T09:00:00-04:00` sorts below every canonical row of that hour while being an hour newer, and 32 qualifying rows were enough to push it out of a window the old `Date.parse` comparator would have picked it from. And `include_recent` is any non-negative finite number at the HTTP edge, so the `Array.slice` this replaced truncated 8.1 to 8 while a SQL `LIMIT` bound 32.4 and threw `datatype mismatch`, turning `/v1/context?pinned_only=1&include_recent=8.1` into a 400. Both are fixed where the constraint was introduced: the count is truncated once at the top of the function, and a store holding any non-canonical `created` takes the whole-corpus load it always had, detected by one covering-index probe against `idx_memories_tenant_created`. Both real stores are fully canonical, 0 of 530 and 0 of 1973 rows, so the measured fast path is the one they still take and the fallback is reserved for a store rebuilt from drifted markdown. The probe itself measured 0.037ms and 0.127ms median over 200 runs against copies of those two stores, against the 43.4ms and 54.7ms the whole load now costs. Validating `include_recent` at the HTTP edge was rejected as the fix, because it would turn a request that has always returned 200 into a 400. Two more cases cover them, and each fails alone if its half is reverted.

### Internal
- The comment above the pinned-budget block in `getContext` still prescribed a fix that the `pinnedReserve` block directly beneath it already implements, so it read as if pins could still be displaced by the recent-N loop. Found by reproduce-checking the TODO that sourced the change above. Comment only, no behaviour change.
