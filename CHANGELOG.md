...
### Performance
- **Every prompt loaded both stores in full to return about eleven rows.** The `UserPromptSubmit` hook installs `hippo context --pinned-only --include-recent 5` (`src/hooks.ts:134`), and that branch reads its working set for exactly three things: the empty-store early return, the pins, and the recent-N candidates. It got that set from two `loadAllEntries` calls, so every prompt read all 530 local and 1896 global rows of the real stores off disk and ran the secret-scanning admission predicate over every one of them. Measured on copies of those stores: 67.7ms per prompt, 32.2ms of it inside the two loads. `loadAmbientCandidates` (`src/store.ts`) replaces them on the pinned-only branch, selecting the pins plus a window of the newest rows in one connection. Interleaved medians over 40 iterations against the same two stores: 77.3ms to 41.8ms with a project set, 81.0ms to 52.7ms without, identical result ids in both. The first attempt was slower than the baseline, 78.3ms, because opening a connection plus `initStore` costs about 5.8ms whatever the row count, so splitting the work into two helper calls paid that floor twice; a widening retry loop that reopened the connection each round reached 118.4ms on a window that mostly failed admission. Both are gone: one connection, and one exhaustive fallback for the case where the whole window is cross-project. The worst case, a local-only store where every row is rejected so the fallback always runs, measured 37.6ms before and 38.3ms after, which is slower and within noise. No migration was needed: `idx_memories_tenant_created` (`src/db.ts:401`) already serves the window, and the pinned query measured 3.34ms unindexed over 1896 rows. `loadAllEntries` is untouched and still serves every other caller, including the non-pinned `getContext` branch. Pinned by `tests/context-pinned-only-load.test.ts`, ten cases over real stores; dropping the pinned query, dropping the fallback, or flipping the `id DESC` tiebreak each fails a different case.
