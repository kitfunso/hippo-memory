...
- **F15 stronger sub-agent rerank on top-100** [next, pivoted]: original F15 prereg (`e4525b6`/`8a88880`) registered a neural cross-encoder rerank (`Xenova/ms-marco-MiniLM-L-6-v2` + `Xenova/bge-reranker-base` via Qdrant fastembed GCS). Task 4 (feasibility spike) discovered the sandbox egress allowlist denies all HF endpoints + all HF mirrors AND the Qdrant fastembed GCS bucket carries embedding models only (verified in `fastembed/rerank/cross_encoder/` source: every reranker has `sources={'hf':'...', 'url': None}`). F11/F13/F14 vendored fine because they're embedders. PyPI alternatives (flashrank, rerankers, fastembed) all wrap HF. Pivoted to a maximally-equipped LLM-as-reranker mechanism (commit `458f006`): Claude Opus 4.7 vs F9's Sonnet/Haiku, top-100 pool vs F9's top-20, 1000-char context vs F9's 600, structured rubric (topical match + evidence specificity + recency-of-claim) vs F9's "rank these", 100 dispatches vs F9's 50. Same input (F14 top-100 on `_s`), same Gate-B threshold (97.7), same structural ceiling (86.2 < 97.7, HARD RETRACTION still the expected outcome). Plan: `docs/superpowers/plans/2026-05-12-f15-cross-encoder-rerank.md` (will be revised in a follow-up commit). Neural cross-encoder track queued as a follow-up conditional on HF egress widening or a pre-downloaded tarball being supplied.
