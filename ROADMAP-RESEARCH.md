...
Cross-track aggregate: **roadmap target R@5 ≥ 85% remains MET on the oracle split as of v1.9.2** (F13 + F9 stack = 86.8). The deployable cross-track best on `data/longmemeval_oracle.json` is still F13 + F9 stack at R@5 = 86.8. On the `_s` split, all three tracks attempted to date (F14 chunked-turn baseline 42.0, F14+F9-Sonnet stack 50.8, F15 Opus rerank 63.6) have Gate-B FAILed against gbrain v0.28.8's 97.6 — F14 and F15 by structurally-expected margins given the BGE-base embedder's R@100 = 86.2 ceiling on the F14 candidate pool. F15 produced the cleanest within-pool measurement: a maximally-equipped LLM-as-reranker (Opus + structured rubric + top-100 + 1000-char context) closes 48.9% of the within-pool ranking gap vs F9's 19.9% — ~2.5× ratio. The embedder remains the dominant lever; F16 (stronger locally-runnable embedder, attacking the R@100 ceiling) is the next track. F17 (text-embedding-3-large via OpenAI) and F18 (fine-tune BGE-base) remain deferred.
