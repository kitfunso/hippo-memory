...
- Keep set: top ceil(budget·N) under (score DESC, memory_id ASC), stable, identical for all scorers; cutoff-boundary score pairs recorded full-precision.
- Clock: ingest per session at `HIPPO_FAKE_NOW` = session `haystack_date`; ALL simulation rounds and extraction at `HIPPO_FAKE_NOW` = `question_date`. Temporal features are pure functions of dataset + seed.
