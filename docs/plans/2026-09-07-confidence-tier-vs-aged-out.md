...
- The suite fails on a defect, not on a date. Added after codex review: the fixtures are dated from a fixed `NOW` while `runCli` spawns the real CLI, so the row seeded one day old would itself have aged out after 2026-10-06 and the status count would read 2 where the case expects 1. `beforeEach` pins `HIPPO_FAKE_NOW` to that same `NOW`, which `evalNow` (`src/ablation.ts:146`) honours in strict canonical form and which the spawned CLI inherits through `process.env`. Checked by moving `NOW` to 2025-01-15: all 16 cases stay green with the pin, and the status count and the MCP label fail without it.
