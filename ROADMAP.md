...
### DF2. Capture distiller stores mid-sentence fragments as high-confidence rules [next]
...
**PREMISE CORRECTED 2026-08-23 (episode 01M0QG886DY), and the correction is the finding.** This entry blamed hard-slicing transcript text before extraction. **Measured false** - the live fragment reproduces byte-identically from untruncated source, and identically under a 500-char and a 120-char slice. Truncation is not involved. That is twice in a row a Part VI entry named a root cause that was inferred rather than executed (DF3 was the first).

**Real cause, plus a second defect nobody had noticed.** Every extraction pattern was an unanchored keyword alternation followed by a fixed-width capture *beginning after the keyword*. So (1) the capture counted characters instead of respecting clause boundaries, producing the reported shrapnel, and (2) **the keyword carrying the negation was discarded, inverting prohibitions**. Measured: `"Never use --no-verify on git commits in this project."` was stored as `"use --no-verify on git commits in this project"` - a real pinned rule, stored as its own opposite, then injected into later prompts. The inversion was never in this entry and is the more serious of the two.

**Shipped:** capture from the keyword; bound to the clause rather than a character count (the terminator must be followed by whitespace, or tokens like `.env` and `capture.ts` split mid-word); keep the 200 ceiling as an upper bound so a clause-free span is truncated-and-stored rather than dropped entirely; and preserve the keyword only when it carries semantic **sign** - labels such as `decision:` or `error:` name a category already recorded on the item, and prefixing them broke AT1's bare-content digest.

**Explicit scope limit:** this does not teach the extractor to tell a rule from narrative. "I have never seen this test fail" still captures, now as a complete correctly-signed clause instead of shrapnel with the negation stripped. That is a judgment problem, not a regex problem.

**Three-strike postmortem - the transferable part.** The clause scanner took three rounds, and every round had the same shape: a hand-rolled character scanner over natural language, shipped against only the cases I had imagined, with adversarial review supplying the one I had not. Round 1 cut inside code delimiters, turning a call expression into a fragment. Round 2 treated every apostrophe as an opening quote, disabling bounding for any contraction or possessive. **Both P1s reintroduced the exact fragment defect the change exists to remove**, and both passed the write gate precisely because code punctuation reads as "specific". Fixed at the cause with one adversarial prose corpus - contractions, possessives, call syntax, object literals, periods in filenames, quoted separators, plain prose - verified load-bearing by reverting each fix and watching it go red.

**Rule earned, and this is a pattern across two consecutive episodes rather than an incident** (DF3's was character classes, DF2's a text scanner): *any hand-rolled scanner or classifier over natural-language text needs an adversarial corpus BEFORE first review, not a case pinned per review round.* Enumerating the cases you can think of is not coverage; it is a list of your own blind spots.
...
**Fix shape:** gate auto-learn ingestion on the audit heuristic; for commit subjects that fail it, either merge subject+body before the check or skip. One-time cleanup of the existing 44 via the audit flow - route through AT3's quarantine once it ships rather than hard-delete, per the "never delete when weakening preserves recoverability" rule.
**Effort:** 1-2d. **Success:** re-running auto-learn on a repo with junk-subject commits stores none of them; a commit whose body carries real detail still stores; `hippo audit` issue count on the live store drops to single digits after cleanup.
