...
async function handleRequest(
  ...
  if (method === 'POST' && path === '/v1/connectors/slack/events') {
    ...
    if (
      ...
      // SAFETY: just confirmed body is a non-null, non-array plain object.
      const bodyRecord = body as Record<string, JsonValue>;
  ...
  if (method === 'POST' && path === '/mcp') {
    ...
    if (
      !mcpReq ||
      Array.isArray(mcpReq) ||
      Object.prototype.toString.call(mcpReq) !== '[object Object]' ||
      Object.prototype.toString.call(mcpReq.method) !== '[object String]'
    ) {
...
export async function serve(opts: ServeOpts): Promise<ServerHandle> {
  ...
  // SAFETY: the check above ruled out null/undefined and the string form of
  // server.address(), leaving the AddressInfo shape Node returns once a TCP
  // socket is bound.
  const addressInfo = address as import('node:net').AddressInfo;
