...
- [ ] **`check-manifest-versions.mjs` lets `package-lock.json` drift, and the gate still prints OK.** Found during the v1.38.9 release: the lockfile auto-merged at 1.38.8, the gate printed "All 4 lockstep manifests + src/version.ts at version 1.38.9. OK.", and the lockfile was bumped by hand after the fact. The exclusion is deliberate (`scripts/check-manifest-versions.mjs:11`, "the lockfile has its own multi-version semantics") but the reasoning is only half right: `.version` and `.packages[""].version` are the root package's OWN version and are lockstep, while everything under `.packages["node_modules/..."]` is the multi-version part the comment describes. Check the two root fields and skip the rest. There is no `npm version`/bump script in this repo, so nothing else updates the lockfile when the manifests are bumped by hand, which is how it drifted. Low harm on its own (npm rewrites the lockfile on install) but the gate reporting OK on a drift it does not inspect is the part worth fixing.
