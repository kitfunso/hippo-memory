...
- [ ] **`hippo auth create-key --role` CLI flag.** v1.12.0 sub-1 added the `role` option to `createApiKey()` but the CLI wrapper doesn't surface `--role admin|member`. Tests construct member keys via direct DB insert. Low-priority follow-up.
- [ ] **`hippo auth list` role column.** v1.12.0 sub-1's `listApiKeys` doesn't include role in the rendered table. Defensive low-priority follow-up.
