...
- [x] **Plain `hippo forget` does not count toward `total_forgotten` when it routes through a server.** FIXED v1.38.8. `hippo remember` had the same defect and was fixed in the same pass: both commands route via `runViaServerIfAvailable` (`src/cli.ts:8828`, `src/cli.ts:9145`), and neither `api.remember` nor `api.forget` counted, so the same command moved the counter or not depending on whether `hippo serve` happened to be up. The filed caveat was wrong: `cmdForget` does not special-case append-only rows before deleting, it catches the trigger's error after `api.forget` throws, so the increment sits after a successful delete on both paths. `api.forget` now counts and `src/cli.ts` no longer does, because its only two callers are the two paths of one user command. `remember` could NOT take that move: `api.remember` is also the bulk write path for `connectors/github/ingest.ts:168`, `connectors/slack/ingest.ts:85` and `importers.ts:970`, so a Slack backfill would flood the user-facing counter; its increment sits at the HTTP route instead, mirroring the CLI surface. Pinned by `tests/cli-thin-client.test.ts` on a real spawned server, mutation-tested on all three arms including the double-count arm.
