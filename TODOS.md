...
- [ ] **`api.recall` never counts `recalled`, so an HTTP recall is invisible in `hippo stats`.** Found alongside the v1.38.9 counter fix. `src/server.ts:930` calls `api.recall` and nothing increments; `cmdRecall` counts at `src/cli.ts:1928`. This is NOT the same defect as remember/forget: `hippo recall` has no `runViaServerIfAvailable` branch, so the CLI command never routes and never loses its count. The HTTP client is simply an uncounted surface, consistently. Deciding whether it should count is a product call on what the counter means, and it drags in MCP, which calls `apiRecall` as a secondary audit band whose length differs from what the caller is shown (`src/mcp/server.ts:538`).
