...
- [ ] **Plain `hippo forget` does not count toward `total_forgotten` when it routes through a server.** `updateStats(hippoRoot, { forgotten: 1 })` sits in the CLI direct path at `src/cli.ts:3696`; neither `src/server.ts` nor `src/api.ts` increments it (the only other `updateStats` outside the CLI is `recalled` at `api.ts:2802`). So `hippo stats` has undercounted routed forgets for as long as forget has routed, and `hippo serve` users see a `total_forgotten` that tracks only the commands that missed the server. v1.38.7 fixed the archive half by moving the increment into `api.archiveRaw`, which both paths call; the same move for plain forget needs the counter inside whatever `api`-level function the forget route shares with `cmdForget`, which is not the same shape (the direct path special-cases append-only rows before it deletes). Found by codex review on PR for v1.38.7.
