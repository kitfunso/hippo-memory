...
- **RESOLVED 2026-09-05 (1.38.2, episode 01M1S8SZH6KZA9FPC9KY92XX5Y): `getHippoRoot` now reuses the bounded ancestor walk from project-identity.ts (home and filesystem root end it; `.hippo` must be a directory), falling back to `<cwd>/.hippo`; the MCP server shares it. Original note:** Local store resolution has no ancestor walk-up. `getHippoRoot(cwd) = cwd/.hippo` strictly (store.ts:255) and `requireInit` hard-exits, so any hippo command run from a SUBDIRECTORY of an initialized project errors ("No .hippo directory found") instead of finding the project root's store - unlike git. Real-usage UX gap; discovered when the S5 fixture design crashed on it. A walk-up (bounded, stop at HOME/drive root) or an explicit opt-in env is the candidate shape; interacts with path-tag attachment (tags derive from cwd, not store root).
- **`hippo init` inside a directory that already has an ancestor store creates nothing (found 2026-09-05, episode 01M1SQ7AD1N4AJKQBMT1AX1DKR).** With the 1.38.2 walk-up, `getHippoRoot` resolves to the ancestor store, so `init` prints `Already initialized at <ancestor>` and no nested store appears; `git init` in the same position creates a nested repo. The micro-eval runner works around it by making its sandbox the home directory. Decide: nested init creates a nested store (git semantics), or keep the reuse and say so in the init output.
- **master has no branch protection, so no CI job blocks a merge (checked 2026-09-05: `gh api repos/kitfunso/hippo/branches/master/protection` is 404).** The `test` and `micro-eval` jobs gate merges only through the deploy ritual (`gh pr checks` before merge). Decide whether to require both as status checks; a repo setting, no code.
...
- **RESOLVED 2026-09-06 (1.38.4, episode 01M1VH406FX7WJEVYMVJFRTAK5): the cause was the pragma ORDER, not either candidate named below. `openHippoDb` ran `PRAGMA journal_mode = WAL` before `PRAGMA busy_timeout = 5000` (`src/db.ts:2366-2367`); `journal_mode` is the first lock-taking statement of every open, so it ran at the default busy timeout of 0 and threw instead of waiting. A 4-arm probe settled it: with the churn poller and the per-open write 4/4 reps crashed, with the write removed 3/4 still crashed, with no churn 0/4, with `busy_timeout` moved first 0/4. So the per-open writes were a REAL but SEPARATE defect (they cost the held-lock path a full 5000 ms wait then a throw) and removing them does not fix this one; both are fixed, each on its own evidence. Checkpoint-on-last-close was never implicated. Original note:** A hippo process closing its last SQLite connection can make a concurrent hippo process's statement fail instantly with `database is locked`, despite `busy_timeout = 5000` (found 2026-09-05, episode 01M1STKYTPFBVWPNDSRZJXYG2V).** Pure `node:sqlite` probe mirroring `openHippoDb`'s pragmas plus the per-open `fts5_available` upsert: a 20 Hz open/upsert/close loop made a single autocommit `UPDATE` in another process fail after 2 ms, 1 time in 10. Every `openHippoDb` runs `runMigrations`, whose `ensureMetaDefaults` and `ensureOptionalFts` write on every open (`src/db.ts:2512-2540`), so every hippo command is a writer. For `session-end` that leaves a stale active snapshot; `loadFreshActiveTaskSnapshot`'s freshness bound is the backstop. Fix at the source: find the BUSY path (per-open writes, checkpoint on last close) and remove it; a retry inside `closeTaskSnapshotsForSession` would be a patch. Evidence: `docs/plans/2026-09-05-session-end-test-observable-signal.md`.
- **RESOLVED 2026-09-06 (1.38.4, episode 01M1VH406FX7WJEVYMVJFRTAK5): `cmdSessionEnd`, `cmdSessionEndWorker` and `cmdCodexSessionEndWorker` are async and await `cmdSleep`; `cmdCodexRun`'s exit handler awaits its inline fallback before the `process.kill`/`process.exit` calls that used to cut it off; the three dispatch sites await. Pinned by `tests/session-end-worker-await-sleep.test.ts`, red at status 1 and green at 0. Original note:** `cmdSessionEndWorker` calls the async `cmdSleep` without `await` inside `try { } catch { }` (`src/cli.ts:3240`), so a rejection escapes the catch and sleep runs alongside capture and the snapshot close in the same process.** Measured 2026-09-05: with no other process touching the DB, the worker still logged `sleep failed: database is locked` in 1 of 5 runs (`poll-log` run 2 in the episode's `ep13-repro-unstressed.log`). Fix: make the worker async and await sleep before capture, then re-check the session-end tests' timing.
- **RESOLVED 2026-09-07 (1.38.10-dev, episode 01M1YACYCW9K9W4S1H1SHY83RZ, PR #184 merged 914284e): `markRetrieved` no longer touches `confidence` at all. A retrieval is a recency signal, so it moves `retrieval_count`, `last_retrieved` and the half-life and leaves the tier to its owner; `resolveConfidence` still derives age-out staleness at read time, so nothing that depended on the derived value changed. Pinned by `tests/invalidation-survives-recall.test.ts`. What is left of this item is the display half, filed separately below: `'stale'` is still both a stored tier and a derived one, so `hippo stats` and `recall --why` cannot tell an aged-out memory from a rejected one. Original note:** One recall permanently un-invalidates a deliberately invalidated memory (found 2026-09-06, episode 01M1VVK5F57RAZP6GRP8CGJQ88, out of scope for PR #174). `invalidateMatching` marks a superseded memory untrusted by setting `confidence = 'stale'`, halving its half-life and tagging it `invalidated` (`src/invalidation.ts:137-141`); `hippo invalidate` does the same at `src/cli.ts:4697`. `markRetrieved` then maps `'stale'` back to `'observed'` on any recall (`src/search.ts:1263`), and both recall paths persist the result: `cli.ts:1890` ("Update retrieval metadata and persist") and `api.ts:2755`, plus the two MCP paths at `mcp/server.ts:636` and `:1101`. So a single recall writes `observed` to disk and the confidence half of the invalidation is gone for good. The `invalidated` tag and the halved half-life survive, which makes the row look untrusted while its tier says otherwise. PR #174 removed the accidental producer of stored `'stale'` (consolidate no longer persists the time-derived value), so `search.ts:1263` now only ever sees deliberate marks and pre-fix rows, which is exactly what makes this reachable case wrong rather than ambiguous. Fix at the source: `markRetrieved` should not decide trust at all. Either leave `confidence` alone and let callers derive, or move the untrust marker off the tier slot entirely (a dedicated stored column, which needs a migration).
...
- [x] **`updateStats` lost increments and clobbered counters it had not been asked to touch.** FIXED v1.38.9. It read all three `meta` counters, added the delta in JS, and wrote all three back through `setMeta`, with no transaction, so a second writer landing between the read and the write was lost, and the unconditional write-back of the two untouched keys stamped their stale values over whatever else had committed. Reproduced with real processes before the fix: four workers doing 40 increments each counted 140 of 160, and two workers on different counters left one at 52 of 60. Now one atomic `INSERT ... ON CONFLICT DO UPDATE SET value = CAST(meta.value AS INTEGER) + CAST(? AS INTEGER)` per counter, and only for the counters the delta names. All nine call sites are unchanged; the fix is in the one shared writer. Pinned by `tests/store-stats-concurrency.test.ts`, which spawns real node processes against one store. This also closes the duplicate item filed by PR #177, which merged into this same 1.38.9 release.
- [x] **Two processes opening a brand-new store at the same time could crash with `database is locked`.** FIXED v1.38.10. The 517 framing here was half the story. Reproduced with 6 real processes released from a barrier onto one cold store: 13 of 30 workers died across 5 trials, and the dominant code was `errcode 5` (`SQLITE_BUSY`), not the 517 this entry named. Stacks pinned three sites, and both root causes are statements SQLite refuses to run the busy handler for, so the `busy_timeout = 5000` set one line above covered neither. First, `PRAGMA journal_mode = WAL` needs an exclusive lock and returns BUSY straight away; every process racing to convert a fresh rollback-journal file to WAL competes and the losers throw. Second, the migration loop's `BEGIN` was DEFERRED, so `migration.up` and `setSchemaVersion` were writes upgrading a read snapshot, which SQLite fails immediately rather than risk a deadlock. The WAL pragma now retries against a 30s deadline, and the loop uses `BEGIN IMMEDIATE` plus a schema-version re-read inside the write lock, because a waiter that re-runs an already-applied migration is not idempotent. `BEGIN IMMEDIATE` was already the repo standard at 14 sites with this exact reasoning written out at `src/store.ts:2026-2035`; `src/db.ts` was the hold-out. The other four deferred-`BEGIN` sites (`src/audit-prune.ts:96`, `src/graph.ts:686`, `src/graph.ts:911`, `src/physics-state.ts:137`) were checked and are clean: each one's first statement is a write or the transaction is read-only, so the lock is taken up front and the handler does fire. After the fix, 0 of 48 and 0 of 72 workers failed, all reaching schema version 41. Pinned by `tests/db-cold-store-concurrency.test.ts`, which was mutation-tested: reverting either half fails 6 of its 8 workers.
...
- [x] **`hippo forget --archive` HTTP route.** SHIPPED v1.38.7. The dispatch skipped server routing for `--archive` on the premise that "the HTTP forget route does not carry it" (noted 2026-05-21), but the dedicated `POST /v1/memories/:id/archive` route and `client.archiveRaw` already existed since `ea155d6` (2026-04-29); the premise was never checked against the code it described. No new route was needed: the `forget` dispatch now calls `client.archiveRaw` on the existing endpoint, the same way it already routes plain forget.
...
- [x] **Plain `hippo forget` does not count toward `total_forgotten` when it routes through a server.** FIXED v1.38.9. `api.forget` now counts and `src/cli.ts` no longer does: its only two callers (`src/cli.ts:3698`, `src/server.ts:1089`) are the two paths of one user command, so no surface can miss it. The filed caveat was wrong: `cmdForget` does not special-case append-only rows before deleting, it catches the trigger's error after `api.forget` throws, so the increment sits after a successful delete on both paths. Pinned by `tests/cli-thin-client.test.ts` on a real spawned server, with an actor assertion on each half so the test cannot pass by running the direct path twice; mutation-tested on both arms including the double-count arm. `remember` was attempted in the same pass and REVERTED, see the item below.

- [ ] **7 of 10 `src/client.ts` exports are never called from `src/cli.ts`.** Measured by `grep -c "client\.$f\b" src/cli.ts` per exported function: only `forget`, `promote`, and `remember` are ever called. `archiveRaw` was the only one with an active false-premise refusal to route and a reproduced harm (fixed in v1.38.7); the other six (`recall`, `supersede`, `authCreate`, `authList`, `authRevoke`, `auditList`) are simply unrouted, with no known false-premise comment. Some are plausibly deliberate (`recall` renders locally and does more than the client call); `supersede` is the closest sibling to the archiveRaw bug (has both a server route and a client function, never routes), but has no false premise in the code and no reproduced harm. Audit each before touching; do not assume they should all route.
- [ ] **A routed `hippo remember` skips the salience gate, so it stores what the direct path would drop.** MEASURED 2026-09-07 on a real spawned server with `salience.enabled: true`: the same text remembered twice went `Remembered` then `Skipped (duplicate, 100% overlap)` with no server, and `Remembered` twice with one up. `richFlag` (`src/cli.ts:8808`) gates routing on `--pin/--global/--extract/--force/--observed/--inferred/`--verified`/`--layer` and has no salience condition, while `api.remember` (`src/api.ts:240`) has no gate at all. The comment above `richFlag` claimed "salience gates still need the direct path" and was false; corrected in v1.38.9 to state the bypass rather than hide it. Same false-premise-in-a-comment class as the v1.38.7 archive bug. Latent for default configs (`salience.enabled` defaults false, `src/config.ts:156`). The fix is a product call: either add salience to `richFlag` so a plain remember stops routing, or apply the gate server-side, which then has to decide what it means for the connector and import callers of `api.remember`. **HUMAN GATE.**
- [ ] **`total_remembered` has no correct home while the routed and direct paths do different things.** Attempted in v1.38.9 and reverted before merge. Counting inside `api.remember` is wrong: it is the bulk write path for `src/connectors/github/ingest.ts:168`, `src/connectors/slack/ingest.ts:85` and `src/importers.ts:970`, so a Slack backfill would flood a user-facing number. Counting at the `POST /v1/memories` handler is also wrong: that route is not a user surface either, since `deploy/aml/adapter/adapter.mjs:213` posts to it for every AML leaderboard add, and `python/src/hippo_memory/client.py:156` is a general SDK on the same route. Blocked behind the salience item above: making the two counters agree while the two paths store different things papers over the real split.
- [x] **A server that commits a write then drops the socket makes the CLI write the row twice.** FIXED, PR #181. The filed framing named `ECONNRESET`; the code Node's fetch actually reports for a destroyed socket is `UND_ERR_SOCKET` on a `fetch failed`, which the old predicate never named and which reached the same wrong answer through its bare-`fetch failed` catch-all. Reproduced with a stub server that commits the row then destroys the socket: 2 rows where there should be 1. The root cause was that `isConnectionRefused` answered "did the transport fail?" while its only consumer, `runViaServerIfAvailable`, read the answer as "is it safe to replay this write locally?". `classifyTransportFailure` (`src/client.ts:270`) now returns `none` / `never-sent` / `delivery-unknown`; the helper falls back only on `never-sent` and on `delivery-unknown` leaves the pidfile alone, says the write may already have landed, and exits non-zero. The closing claim on the old entry was half right: an idempotency key is what would let the CLI safely RETRY, and that is still not built. It was never needed to stop the silent duplicate, because "definitely never arrived" is distinguishable from "unknown" and that is the only distinction the fallback decision needs. Sibling audit: `src/cli.ts:349` is the only HTTP-to-direct fallback in the tree.
- [x] **`updateStats` read-modify-writes all three counters with no transaction** (`src/store.ts:2323-2329`). A `hippo serve` daemon and a direct-path CLI process running concurrently can lose an increment: each reads the old value and writes back its own. Pre-existing shape, more reachable now that a long-running writer also calls it. FIXED v1.38.9. Same defect as the store-concurrency item above: PR #177 filed it, PR #178 fixed it, and both landed in this release.
- [x] **`api.recall` never counts `recalled`, so an HTTP recall is invisible in `hippo stats`.** Fixed in PR #183, at the route rather than in `api.recall`. Found alongside the v1.38.9 counter fix. `src/server.ts:930` calls `api.recall` and nothing incremented; `cmdRecall` counts at `src/cli.ts:1938`. This is NOT the same defect as remember/forget: `hippo recall` has no `runViaServerIfAvailable` branch, so the CLI command never routes and never lost its count, which is also why the new route-level counter cannot double-count it. The product call landed on "count it": every other read surface already does, since `GET /v1/context` routes to `api.getContext`, which counts at `src/api.ts:2818`. `api.recall` was the wrong home for it, because the CLI never calls that function and MCP calls it for a secondary audit band whose length differs from what the caller is shown (`src/mcp/server.ts:544`); the counter therefore sits in the route, next to the response it describes. Reproduced before the fix on a real server: three memories stored, `GET /v1/memories?q=alpha` returned 3 results, `total_recalled` stayed at 0. Drill is deliberately still uncounted on both surfaces (`cmdDrillDown` at `src/cli.ts:7427` and `GET /v1/recall/drill/:id`), so that pair stays symmetric. Pinned by `tests/http-recall-counter.test.ts`; removing the call fails 4 of its 5 cases. The MCP counter question is untouched and still open below.
- [ ] **`total_recalled` and its siblings are hand-maintained per surface, and that has now cost three bugs in three releases.** Archive in v1.38.7, plain forget in v1.38.9, HTTP recall in the release after. Each fix added a counter at one more site; none removed the reason a new surface starts uncounted. Every recall already writes an audit row (`op: 'recall'`, `src/api.ts:986`), so the counters could be derived from the audit log instead of incremented by hand, and a new surface would then count for free. NOT a drop-in change and it needs sign-off: `bootstrapLegacyStore` (`src/store.ts:1129`) seeds `total_recalled` from legacy stats, so the counter carries history predating the audit table, and deriving it would silently reset a number users already read out of `hippo stats`. That makes it a breaking change to a published surface, needing either a migration that backfills or an explicit decision to reset. Also settle at the same time whether "recalled" counts memories returned (what all three sites do today) or recall calls.
- [x] **`'stale'` is both a stored confidence tier and a derived one, and the reporting surfaces cannot tell them apart.** Done additively (v1.38.10): `confidenceFacets` splits the pair and the eight display sites render both. Removing `'stale'` from `ConfidenceLevel` is still open and still needs sign-off, for the reason given at the end of this item. What remains of AT2 (`ROADMAP.md:1084`) after the recall-undoes-invalidation fix. `resolveConfidence` (`src/memory.ts:447`) returns the literal `'stale'` for any non-pinned, non-verified entry untouched for 30 days, discarding the stored `observed`/`inferred` tier, and `'stale'` is also what `hippo invalidate` and decision supersede store. So `hippo stats` buckets an aged-out well-grounded memory under "Stale" beside a deliberately rejected one, and `hippo recall --why` reports `confidence: "stale"` for a row whose stored tier is `observed`. `hippo inspect` already renders it correctly: `Confidence: observed (effective: stale)` at `src/cli.ts:3737`, so the shape exists at one site and nowhere else. Measured 2026-09-07: on `~/hippo/.hippo` 5 rows of 530 differ between stored and displayed tiers, and on `~/.hippo` none do, because the prompt hook recalls often enough to keep `last_retrieved` fresh. Small live footprint, so it is a reporting-correctness item rather than an urgent one. Two ways to take it: additively, with a `{ tier, stale }` facet helper used at the 8 display sites and `resolveConfidence` left alone, or properly, by removing `'stale'` from `ConfidenceLevel` and giving invalidation its own state. The second needs sign-off: `ConfidenceLevel` is exported from `src/index.ts` and 807 live rows across the two stores hold the literal `'stale'`, so it is a breaking change plus a data migration.
- [ ] **A retrieval surface reports state the retrieval itself has already changed, and the two retrieval commands disagree about which state.** Found in v1.38.10 while adding `aged_out`. `markRetrieved` (`src/search.ts:1254`) sets `last_retrieved` to now for every row a retrieval returns. `hippo recall` renders the pre-refresh entry, so it can print `[observed, aged]` and `aged_out: true`; `hippo context` renders the post-refresh copy, because `src/api.ts:2820` deliberately swaps in the `markRetrieved` results "so the returned ContextResult reflects post-recall state", so it prints `[observed]` for the same row in the same second. Both are defensible readings of "what is this memory's state", and the same fork decides whether `strength` in context output is pre- or post-boost, which is why the swap exists. v1.38.10 sidestepped it: `aged_out` means "at the moment of retrieval", it ships on `recall --why`, `explain --json`, `trace` and the dashboard, and it was deliberately NOT added to `context --format json` where it could only ever be `false`. Deciding it properly means picking one semantics for every retrieval surface and is a change to the highest-traffic path in the product, so it needs sign-off. Not urgent: the human-facing labels have disagreed this way since the recall boost shipped.
- [ ] **No MCP operation moves any stats counter.** `src/mcp/server.ts:30` imports `apiRecall`/`apiRemember` and neither counts. Unlike the routed CLI case this is consistent rather than uptime-dependent, so it is a policy question: should an agent's MCP remember show up in the same `total_remembered` a human's `hippo remember` does?
- [x] **`check-manifest-versions.mjs` lets `package-lock.json` drift, and the gate still prints OK.** Fixed in PR #182. Found during the v1.38.9 release: the lockfile auto-merged at 1.38.8, the gate printed "All 4 lockstep manifests + src/version.ts at version 1.38.9. OK.", and the lockfile was bumped by hand after the fact. The exclusion was deliberate (`scripts/check-manifest-versions.mjs:11`, "the lockfile has its own multi-version semantics") and the entry's reading of it held up: `.version` and `.packages[""].version` are the root package's own version and are lockstep, while everything under `.packages["node_modules/..."]` is the multi-version part the comment described. The gate now checks the two root fields and still ignores the dependency entries, and reports a missing lockfile, a parse error and a missing version field rather than skipping them. The failure message names `npm install --package-lock-only`, because there is no `npm version`/bump script in this repo, so nothing else rewrote the lockfile when the manifests were bumped by hand, which is how it drifted. Pinned by `tests/manifest-version-gate.test.ts`, which spawns the real script against a minimal repo in a temp dir; reverting the new block fails 3 of its 5 cases. Sibling audit found no second instance: `ui/`, `website/` and `extensions/claude-code-plugin/` are independent cadences already named in the gate header, `evals/model-profile-bench.json` is an eval artifact, and `python/pyproject.toml` is `hippo-memory-sdk` at 0.3.0 with its own bump history, so none of them belong in `LOCKSTEP_MANIFESTS`.
