...
- [ ] **A server that commits a write then drops the socket makes the CLI write the row twice.** `isConnectionRefused` (`src/client.ts:267-284`) returns true for a `cause.code` of `ECONNRESET` and for a bare `fetch failed`, both of which a mid-response socket drop produces after the server has already committed. `runViaServerIfAvailable` then returns false and the direct path writes a second row. Pre-existing and independent of the counters; the transport cannot distinguish "never arrived" from "arrived and the reply was lost" without an idempotency key on the request.
