...
- **Graph-hop scope parity (cross-surface).** `--hops` / `--graph-stream` expansion loads reached memories by id and re-applies superseded/asOf filters but NOT the recall scope rule; api.recall's hops path has the same shape, so this is cross-surface parity, not a CLI residue. Audit whether consolidated/E2-provenance graph nodes can carry private scopes at all; if yes, add the scope predicate to the expansion loaders.
